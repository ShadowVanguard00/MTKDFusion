...
def trainer(train_loader, 
                       ...
                       net_g_tea3,
                       net_tea_fuse,
                       device, optimizer,  logger, start_ep=0, total_it=0):
    total_epoch = 50
    start = glob_st = time()
    for ep in range(start_ep, total_epoch):
        for it, (img_ir, img_vi) in enumerate(train_loader):
            total_it += 1
            ...
            with torch.no_grad(): 
                ...
                tea3_fused_img, _= net_g_tea3_model_D(F, emb_F)
            ...
            train_loss.backward()
            nn.utils.clip_grad_norm_(net_tea_fuse.parameters(), max_norm=0.1, norm_type=2)
            optimizer.step()
        ...
        if (ep + 1) % 10 == 0:
            torch.save(
                ...
                os.path.join('./result', f"net_tea_fuse_epoch_{ep+1}.pth"))
...
class ka_model(nn.Module):
    ...
    def optimize_parameters(self): 
        ...
        train_dataset = MSRSData(is_train=True)
    ...
    def test_one_dataset_weight(self):
        ...
        Fusion_save_dir = './fused_img/MSRS'
        print(Fusion_save_dir)
        os.makedirs(Fusion_save_dir, exist_ok=True)

        self.net_tea_fuse.load_state_dict(torch.load("./result/model.pth"))
...
if __name__ == '__main__':
   ...
   ka_instance.test_one_dataset_weight()