...
parser.add_argument('--checkpoint', type=str, default='/home/NewPartion/hlk/MKDFusion_new/tea_training/tea_weights/Final.model', help='fusion network weight')
...
def trainer(train_loader, 
    ...
    for ep in range(start_ep, total_epoch): ## 每一个epoch 计算一次动态权重   
        ref1_weight = 0.1 + (0.5 - 0.1) * (ep / 50)  # 从0.1线性增长到0.5
        ...
        for it, (img_ir, img_vi, label) in enumerate(train_loader):
            ...
            # tea_fused_img=(tea_fused_img-torch.min(tea_fused_img))/(torch.max(tea_fused_img)-torch.min(tea_fused_img))
            ...
            # train_loss = 0.1 * fusion_loss + ref1_loss + ref2_loss + ref3_loss
            # print('fusion_los:{}, ref1_loss:{}, ref2_loss:{}, ref3_loss:{}'.format(0.1 * fusion_loss, ref1_loss, ref2_loss, ref3_loss))       
            train_loss = 0.1 * fusion_loss + ref1_weight * ref1_loss + ref2_loss + ref3_loss
            print('fusion_los:{}, ref1_loss:{}, ref2_loss:{}, ref3_loss:{}'.format(0.1 * fusion_loss, ref1_weight * ref1_loss, ref2_loss, ref3_loss))
...
class ka_model(nn.Module):
    ...
    def test_one_dataset_weight(self):
        ...
        Fusion_save_dir = '/home/NewPartition/hlk/MTKDFusion_ablation/test_ablation/fused_imgs_MSRS/knowledge_distillation'
        print(Fusion_save_dir)
        os.makedirs(Fusion_save_dir, exist_ok=True)

        self.net_tea_fuse.load_state_dict(torch.load("/home/NewPartion/hlk/MKDFusion_new/tea_training/results_bt8/net_tea_fuse_epoch_50.pth"))
