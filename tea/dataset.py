    ...
    def __init__(self, train_dirs_ir, train_dirs_vi, train_dirs_gt, is_patch=True, transform=None):
        ...
        for ir_dir, vi_dir, gt_dir in zip(train_dirs_ir, train_dirs_vi, train_dirs_gt):
            ...
            for name in common_names:
                ...
                if ir_img is None:
                    print(f"跳过读取失败文件: {ir_path}")
                    continue
                if vi_img is None:
                    print(f"跳过读取失败文件: {vi_path}")
                    continue
                if gt_img is None:
                    print(f"跳过读取失败文件: {gt_path}")
                    continue

                # 检查尺寸是否都是 768x1024
                if ir_img.shape[:2] == (self.target_h, self.target_w) and \
                   vi_img.shape[:2] == (self.target_h, self.target_w) and \
                   gt_img.shape[:2] == (self.target_h, self.target_w):
                    ...
                    print(
                        f"跳过尺寸不匹配文件: {name} | "
                        f"IR={ir_img.shape[:2]}, VI={vi_img.shape[:2]}, GT={gt_img.shape[:2]}"
                    )
...
class Get_Patch_Dataset_RGB(Dataset):
    def __init__(self, train_dirs_ir, train_dirs_vi, train_dirs_gt, is_patch=True, transform=None):
        super(Get_Patch_Dataset_RGB, self).__init__()
        ...
        for ir_dir, vi_dir, gt_dir in zip(train_dirs_ir, train_dirs_vi, train_dirs_gt):
            ...
            for name in common_names:
                ...
                h_vi, w_vi = vi_img.shape[:2]
                h_gt, w_gt = gt_img.shape[:2]

                if min(h_ir, w_ir, h_vi, w_vi, h_gt, w_gt) < 256:
                    print(
                        f"跳过尺寸过小文件: {name} | "
                        f"IR={ir_img.shape[:2]}, VI={vi_img.shape[:2]}, GT={gt_img.shape[:2]}"
                    )
