...
class Saver():
  ...
  def write_img(self, ep, input, output):
    input = torch.cat((input[0][:1, ::].repeat(1, 3, 1, 1), input[1][:1, ::], input[2][:1, ::].repeat(1, 3, 1, 1), input[3][:1, ::].repeat(1, 3, 1, 1), input[4][:1, ::].repeat(1, 3, 1, 1), input[5][:1, ::].repeat(1, 3, 1, 1)), 3)
    pred = torch.cat((output[0][:1, ::].repeat(1, 3, 1, 1), output[1][:1, ::], output[2][:1, ::], output[3][:1, ::], output[4][:1, ::], output[5][:1, ::]), 3)
